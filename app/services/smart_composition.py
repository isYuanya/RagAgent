...
def create_run(payload: SmartCompositionRunCreate) -> SmartCompositionRunDetail:
    ...
    try:
        ...
        composition = compositions.generate_auto_composition(
            AutoCompositionRequest(brief=_composition_brief(payload.brief))
        )
        ...
        _set_step(
            ...
            "knowledge_retrieval",
            "completed",
            message=f"Retrieved {len(composition.reference_fragments)} reference fragments.",
        ...
        if payload.mode == "guided":
            _set_step(run, "composition_selection", "waiting_for_user", message="Waiting for user confirmation.")
            run.status = "waiting_for_user"
            return _save_run(run)

        _finish_auto_run(run)
        run.status = "finished"
        return _save_run(run)
