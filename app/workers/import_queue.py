...
def enqueue_copy_import(csv_text: str) -> TaskResponse:
    task = create_task(
    ...
    try:
        ...
        progress = task.progress.model_copy(
            update={"current_message": f"任务已入队：{job.id}。"}
        )
        return set_task_running(task.task_id, progress)
    except RedisError:
        return run_copy_import_task(csv_text, task.task_id)


def enqueue_text_import(text: str) -> TaskResponse:
    task = create_task(
        progress=TaskProgress(
            phase="queued",
            model=settings.openai_model,
            current_message="Plain text import task created; waiting for worker.",
        )
    )
    try:
        queue = get_queue("copy_import")
        ...
        progress = task.progress.model_copy(
            update={"current_message": f"Task queued: {job.id}."}
        )
        return set_task_running(task.task_id, progress)
    except RedisError:
        return run_text_import_task(text, task.task_id)
